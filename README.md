...
## Purpose
We present *LEGOS-SLEEC*, a tool designed to support interdisciplinary stakeholders in specifying normative requirements as <em>SLEEC rules</em> (social, legal, ethical, empathetic, and cultural domain-specific language[^1]), and in analyzing and debugging their well-formedness. *LEGOS-SLEEC* is built using four previously published components (FOL* satisfiability checker LEGOS[^2] SLEEC to FOL* translator[^3], FOL* wellformedness satisfiability checker[^4], and FOL* UNSAT proof analyzer[^5]), which have been shown to be effective and usable across nine case studies[^4]. Reflecting on this experience, we have significantly improved the user interface of *LEGOS-SLEEC* and its diagnostic support, and demonstrate the effectiveness of these improvements using four interdisciplinary stakeholders[^6]. Showcase video URL is [HERE](https://youtu.be/LLaBLGxSi8A).
...
## Provenance
The artifact is available [here](https://zenodo.org/records/14714867) on Zenodo, and the paper is available [here](https://arxiv.org/abs/2501.12544).
...
4. Click on the situational conflict logo as shown below:  
   ...
   and select the `SAFESCADE.sleec` case-study. 
6. You can now compare the diagnostics generated by the two versions of the tool. Refer to the image below to see the expected outcomes:  
...
7. To resolve the first issue, edit the rule R4 to become:
  ```
